...
<!-- FIND VERSION -->
**This README is for the `rubocop-lts` 0.x line supporting Ruby >= 1.8.**

Use this branch for released applications whose oldest supported Ruby is 1.8.
For a different Ruby floor, choose the matching branch and major version line
...
Compatible with MRI Ruby 1.8+ for this release line, and concordant
releases of JRuby and TruffleRuby.
CI workflows and Appraisals may run on newer Rubies when the current toolchain
cannot run on the oldest supported interpreter.
This branch is intended to protect projects whose dependency floor still
includes Ruby 1.8.
...
```console
bundle add rubocop-lts --version "~> 0.3"
```

If bundler is not being used to manage dependencies, install the gem by executing:

```console
gem install rubocop-lts --version "~> 0.3"
```
...
```ruby
spec.add_dependency("rubocop-lts", "~> 0.3")
```
